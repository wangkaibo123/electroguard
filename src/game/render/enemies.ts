...
const drawEnemyBody = (
  ...
  const fillAlpha = 'rgba(' + hexToRgb(eColor) + `,${e.isPreview ? 0.12 : 0.2})`;

  ctx.save();
  if (e.isPreview) ctx.globalAlpha = 0.7;
  ...
  if (e.isPreview) {
    ctx.setLineDash([5, 5]);
    ctx.strokeStyle = e.isStatic ? 'rgba(255,255,255,0.55)' : 'rgba(248,113,113,0.5)';
    ctx.lineWidth = 1;
    ctx.beginPath();
...
export const drawEnemyPreview = (
  ctx: CanvasRenderingContext2D,
  preview: { x: number; y: number; enemyType: EnemyType; isStatic: boolean } | null,
  now: number,
) => {
  if (!preview) return;

  const radiusByType: Record<EnemyType, number> = {
    scout: 9,
    grunt: 12,
    tank: 15,
    saboteur: 11,
  ...
  drawEnemyBody(ctx, {
    x: preview.x,
    y: preview.y,
    enemyType: preview.enemyType,
    heading: 0,
    radius: radiusByType[preview.enemyType],
    color: colorByType[preview.enemyType],
    shieldAbsorb: preview.enemyType === 'overlord' ? 1 : 0,
    maxShieldAbsorb: preview.enemyType === 'overlord' ? 1 : 0,
    isPreview: true,
    isStatic: preview.isStatic,
  }, now);
};
